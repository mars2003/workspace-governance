...
- 具备文件/目录扫描与操作能力（查看、移动、重命名、归档、删除）
- 具备日志输出能力（文件或结构化输出）
- 具备用户确认能力（用于 destructive/ask-user 场景）
- 必须先确定 `workspace_root`（用户提供或默认当前目录）

### `SKILL_ADAPT` 接入要求

- 若存在 `SKILL_ADAPT.yaml`，执行前必须读取
- 配置优先级：用户显式指令 > `SKILL_ADAPT` > 仓库现有约定 > 技能默认保守策略
- 配置解析失败时不得静默忽略，需回退默认并记录 warning

### 非交互环境安全策略

针对 cron、后台子代理、仅执行环境：

- 遇到 `ask-user` 项必须停止并标记 `blocked`
- 必须输出 `pending_decisions` 待决策清单
- 禁止静默跳过、自动审批或自动删除歧义项
- 无确认能力时，destructive 动作必须 `fail-fast`

### 回滚与检查点最小规范

- 执行前检查点：`checkpoint_before`
- 每批次检查点：`checkpoint_batch_<n>`
- 执行后检查点：`checkpoint_after`
...
`workspace-governance` 可以独立运行，但以下场景可选接入其他 skill/tool：

- 涉及 Git/GitHub 的归档校验：接入 Git/GitHub 能力
- 涉及云端归档生命周期：接入云存储能力
- destructive 批次需审批流：接入流程/审批能力

规则：

- 不默认假设其他 skill 已存在
- 用户意图需要但依赖缺失时，必须标记 `blocked` 并输出缺失依赖

## 十五、维护策略（避免文档腐化）

- `SKILL.md` 是执行语义单一真源（single source of truth）
- `reference/` 手册用于解释和示例，不应新增与 `SKILL.md` 冲突的规则
- 每次变更规范时，先改 `SKILL.md`，再同步手册
